"""Sherlock live CLI adapter with explicitly selected offline test fixtures."""
    ...
    def normalize(self, raw: Any) -> ConnectorResult:
        ...
        if not isinstance(raw, list) or not all(isinstance(item, dict) for item in raw):
            raise ConnectorNormalizationError(
                "sherlock mock payload must be a list of fixture records"
            )

        profiles = [candidate_from_fixture(item, self.name) for item in raw]
        observations = [
            existence_observation(profile, self.name, reliability=0.75) for profile in profiles
        ]
...
MockSherlockConnector = SherlockConnector
